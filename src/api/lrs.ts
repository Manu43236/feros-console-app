...
import type { ApiResponse, Lr, LrCharge, LrCheckpost, LrStatus, PageResponse } from '@/types'
...
export const lrsApi = {
  getAll: (params?: { page?: number; size?: number; search?: string; status?: string }) =>
    apiClient.get<ApiResponse<PageResponse<Lr>>>('/lrs', { params }).then(r => r.data),
  ...
  deleteCharge:    (id: number, chargeId: number) => apiClient.delete<ApiResponse<void>>(`/lrs/${id}/charges/${chargeId}`).then(r => r.data),
}
