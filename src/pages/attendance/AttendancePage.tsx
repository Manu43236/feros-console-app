            ...
            <tbody className="divide-y">
              {pageRows.map(r => (
                <tr key={r.id} className={cn('hover:bg-gray-50', selected.has(r.id) && 'bg-blue-50')}>
                  ...
                  <td className="px-5 py-3 w-[140px]">{r.locationName ? <span className="block w-[140px] truncate text-gray-500 text-xs" title={r.locationName}>{r.locationName}</span> : r.latitude != null && r.longitude != null ? <a href={`https://www.google.com/maps/search/?api=1&query=${r.latitude},${r.longitude}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-blue-600 hover:underline"><MapPin size={11} />Other</a> : <span className="text-gray-400 text-xs">—</span>}</td>
